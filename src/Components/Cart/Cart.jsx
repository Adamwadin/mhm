...
const stripePromise = loadStripe("pk_test_51QdysO08pnSrIpHIdCR8CchdN25EwwuW1VgK8ndNYFWLgWyb8OFFW6AaCs5XhUisBSP1Lze9vCDN2iwNuSITadr100KpxPDKfW");
    ...
    <div>
                ...
                return (
                  <li key={index}>
                    <div>
                      ...
                      <span><strong>Guest:</strong> {item.booked_by}</span>
                      <span><strong>Cleaning fee:</strong> {mandatoryCleaningFee}$</span>
                      <span><strong>Dates:</strong> {startDate.toLocaleDateString()} - {endDate.toLocaleDateString()}</span>
                    </div>
                    <button className={styles.removeButton} onClick={() => removeFromCart(index)}>Remove booking</button>
                  </li>
                );
              })}
            </ul>
      ...
      {isCheckoutFormModalOpen && (
        <Elements stripe={stripePromise}>
          <CheckoutForm totalAmount={totalAmount} onClose={handleCloseCheckoutFormModal} orderItems={cartItems} />
        </Elements>
      )}
    </div>
  );
